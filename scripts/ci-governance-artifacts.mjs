...
export function validateArtifacts(root, base, head) {
  ...
  if (diff.status !== 0)
    throw new Error(`Cannot determine changed specs: ${diff.error?.message ?? diff.stderr}`);
  const inside = file => {
    const resolved = fs.realpathSync(file);
    if (!resolved.startsWith(root + path.sep))
      throw new Error(`Artifact escapes checkout: ${file}`);
    return resolved;
  };
  const specs = [];
  for (const file of diff.stdout.split('\0').filter(Boolean)) {
    if (!/^\.caws\/specs\/.*\.ya?ml$/.test(file) || file.startsWith('.caws/specs/.archive/'))
      continue;
    const messages = [];
    const code = runSpecsValidateCommand({
      file: inside(path.join(root, file)),
  ...
  return {
    ok:
      specs.every(spec => spec.ok) &&
      ![...corpus.diagnostics, ...waivers.diagnostics].some(d => d.severity === 'error'),
    specs,
    specCount: corpus.specs.length,
    specDiagnostics: corpus.diagnostics,
    waiverCount: waivers.waivers.length,
    waiverDiagnostics: waivers.diagnostics,
  };
}
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  try {
    if (process.argv.length !== 4)
      throw new Error('Usage: ci-governance-artifacts.mjs <base-sha> <head-sha>');
