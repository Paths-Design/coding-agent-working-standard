...
test('CI validates portable artifacts without inventing local worktree state', () => {
  ...
  for (const key of Object.keys(env))
    if (key.startsWith('GIT_') && !['GIT_CONFIG_GLOBAL', 'GIT_CONFIG_SYSTEM'].includes(key))
      delete env[key];
  const git = (...args) =>
    execFileSync(
      'git',
  ...
  try {
    ...
    const spec = {
      ...
      blast_radius: { modules: ['ci'], data_migration: false },
      operational_rollback_slo: '5m',
      scope: { in: ['src'], out: [] },
      invariants: ['Preserve source'],
      acceptance: [{ id: 'A1', given: 'source', when: 'validated', then: 'checked' }],
      non_functional: {},
      contracts: [],
    };
    const specFile = write('specs/CI-FIXTURE-001.yaml', spec);
    const waiver = {
      id: 'CI-VALID-WAIVER-001',
      title: 'Fixture waiver',
      status: 'active',
      gates: ['budget_limit'],
      reason: 'Fixture',
      approved_by: 'fixture',
    ...
    assert.equal(
      validateArtifacts(root, base, head).ok,
      false,
      'duplicate IDs outside the changed-file list must block'
    );
    ...
    fs.symlinkSync('/etc/hosts', specFile);
    assert.throws(() => validateArtifacts(root, base, head), /escapes checkout/);
    assert.throws(
      () => validateArtifacts(root, 'missing-ref', head),
      /Cannot determine changed specs/
    );
  } finally {
