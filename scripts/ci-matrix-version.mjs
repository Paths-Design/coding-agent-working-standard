...
const versionOf = tag => {
  if (typeof tag !== 'string' || tag.trim() !== tag)
    throw new Error('Release tag must be a single canonical value');
...
export function resolveTarget(eventName, event, view) {
  ...
  if (eventName === 'workflow_run') {
    ...
    if (typeof requested !== 'string' || requested.trim() !== requested)
      throw new Error('Invalid manual version');
  ...
  if (/^[0-9]/.test(requested) && version !== requested)
  ...
  return {
    ...
    source: eventName,
    releaseRunId: event.workflow_run?.id ?? null,
  };
}
...
export function main(env = process.env) {
  ...
  const target = resolveTarget(env.GITHUB_EVENT_NAME, event, spec => {
    const r = spawnSync(
      'npm',
      ['view', spec, 'version', 'gitHead', 'dist.integrity', '--json', '--workspaces=false'],
