...
test.each(['pre_tool_use', 'post_tool_use', 'session_start', 'stop', 'pre_compact'])(
  ...
  (event) => {
    ...
    const previous =
      reference.slice(0, tail) +
      `run_handlers${event === 'pre_tool_use' ? ' --short-circuit-on-block' : ''} "${'${HANDLERS[@]}'}"\n`;
    ...
    expect(() =>
      extractMachineHandlers(previous.replace('run_handlers ', 'run_custom_handlers '), reference)
    ).toThrow(/Custom dispatcher logic/);
  }
);
