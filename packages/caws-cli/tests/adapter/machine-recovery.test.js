...
test('machine recovery targets canonical project state and never writes into its snapshot', () => {
  ...
  try {
    ...
    const invoke = (project) =>
      spawnSync(
        'bash',
        [
          path.join(runtime, 'reset-danger-latch.sh'),
          '--session',
          'fixture',
          '--reason',
          'fixture recovery',
    ...
    const audit = fs
      .readFileSync(path.join(repo, '.claude/logs/danger-latch-resets.log'), 'utf8')
      .trim();
    expect(JSON.parse(audit).reason).toBe('fixture recovery');
    expect(fs.existsSync(path.join(home, 'lib/.claude'))).toBe(false);
    expect(verifyRuntime(home, installed.digest)['reset-danger-latch.sh']).toMatch(
      /^[a-f0-9]{64}$/
    );
  } finally {
...
test.each([
  ...
  'the emitted recovery command preserves project and session boundaries: %j',
  ({ machine, linked }) => {
    ...
    const latchIn = (project, id) =>
      path.join(project, '.claude/hooks/state', `danger-latch-${id}.json`);
    ...
    try {
      ...
      const reset = spawnSync('/bin/bash', ['-c', command], {
        cwd: unrelated,
        env: human,
        encoding: 'utf8',
      });
      ...
      expect(JSON.parse(fs.readFileSync(unrelatedLatch, 'utf8')).reason).toBe(
        'independent control'
      );
