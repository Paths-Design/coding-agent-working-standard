...
const workflow = yaml.load(
  fs.readFileSync(path.join(pkgRoot, '../../.github/workflows/pr-checks.yml'), 'utf8')
);
...
function instrumentCount(script) {
  ...
  return Number(
    execFileSync(bats, ['--count', '--filter', '^environment sanity: /bin/bash', selection[1]], {
      cwd: pkgRoot,
      encoding: 'utf8',
    }).trim()
  );
...
test('the macOS CI suite still selects the mandatory Bash instrument', () => {
  ...
  expect(
    workflow.jobs.hook_bats_bash32_macos.steps.some(
      (step) => step.run === 'npm run test:bats:macos'
    )
  ).toBe(true);
