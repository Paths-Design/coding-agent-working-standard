...
test('heartbeat caches are separate from leases without hiding malformed sessions', () => {
  ...
  try {
    ...
    fs.mkdirSync(dir);
    const files = {
      'heartbeat-emit-state.json':
        '{"peer_set_hash":"fixture","peer_count":2,"last_emitted_ts_ms":1}',
    ...
    expect(result.value.diagnostics.map((d) => path.basename(d.subject))).toEqual([
      'broken-session.json',
    ]);
    for (const [name, bytes] of Object.entries(files))
      expect(fs.readFileSync(path.join(dir, name), 'utf8')).toBe(bytes);
    ...
    expect(loadLeases(root).value.diagnostics.map((d) => path.basename(d.subject))).toEqual([
      'broken-session.json',
    ]);
