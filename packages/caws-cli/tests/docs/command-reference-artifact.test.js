...
test('generation writes only package artifacts, rejects stale output and requires the local build', () => {
  ...
  try {
    ...
    for (const file of [script, metadata, source])
    ...
    fs.writeFileSync(
      metadata,
      `exports.COMMAND_SURFACE_METADATA = ${JSON.stringify([
        {
          kind: 'group',
          name: 'init',
          description: 'Set up CAWS',
          subcommands: [
            {
              kind: 'group',
              name: 'adapters',
              description: 'Machine adapters',
              subcommands: [
                {
                  kind: 'leaf',
                  name: 'example',
    ...
    expect(fs.readFileSync(output, 'utf8')).toContain(
      'caws init adapters example <first> [second]'
    );
