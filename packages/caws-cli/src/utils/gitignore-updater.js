 ...
 * - .caws/specs/*.yaml (per-feature specs — vNext is multi-spec only)
 * - .caws/policy.yaml (team policy)
 * - .caws/waivers/*.yaml (project-wide waivers)
 * - .caws/plans/*.md (implementation plans)
 *
 * IGNORED (local-only runtime telemetry, generated per-commit/session):
 * - .agent/ (agent runtime tracking, local to each developer)
 * - .caws/state/ (per-spec working state, auto-managed)
 * - .caws/worktrees/ + worktrees.json (worktree registry, local)
 * - .caws/agents.json (session/heartbeat telemetry)
 * - .caws/events.jsonl (hash-chained event log, local audit trail)
 * - .caws/sessions/ (session capsules)
 * - .caws/provenance/ (legacy provenance chain — superseded by events.jsonl)
...
# ====================================================================
# Tracked:  .caws/specs/, .caws/policy.yaml, .caws/waivers/, .caws/plans/
# Ignored:  runtime telemetry (events, agents, worktrees, state, sessions),
#           tools, temp files, logs, local overrides
...
# CAWS Worktree registry (local, per-clone)
.caws/worktrees/
.caws/worktrees.json

# CAWS session/heartbeat telemetry (local, per-shell)
.caws/agents.json
.caws/sessions/

# CAWS event log (local audit trail, hash-chained per-clone)
.caws/events.jsonl
.caws/events.jsonl.lock

# CAWS legacy provenance chain (superseded by events.jsonl in vNext)
.caws/provenance/

# Claude hook runtime state (dangerous-command latches, strike counters).
# Latch state is per-session and must not be committed.
.claude/hooks/state/
