...
const ajv = new Ajv();
const workingSpecSchema = require(path.join(TEMPLATE_DIR, ".caws/schemas/working-spec.schema.json"));
const validateWorkingSpec = ajv.compile(workingSpecSchema);
  ...
  const template = {
    ...
    acceptance: answers.acceptanceCriteria
      ...
      .map((criteria, index) => {
        ...
        if (upperCriteria.includes("GIVEN") && upperCriteria.includes("WHEN") && upperCriteria.includes("THEN")) {
 ...
 */
function validateGeneratedSpec(specContent, answers) {
  try {
  ...
  try {
    ...
    const questions = [
      ...
      },
      {
        ...
        default: (answers) => {
          if (answers.projectMode === "feature") return "legacy authentication, deprecated endpoints";
          if (answers.projectMode === "refactor") return "external dependencies, configuration files";
    ...
    const provenanceData = {
      ...
      toolAllowlist: ["node", "npm", "git", "fs-extra", "inquirer", "commander", "js-yaml", "ajv", "chalk"],
    ...
    if (options.git) {
      try {
        ...
        require("child_process").execSync(
          'git commit -m "Initial CAWS project setup"',
          { stdio: "inherit" }
        );
        console.log(chalk.green("✅ Git repository initialized"));

        // Update provenance with commit hash
        const commitHash = require("child_process").execSync("git rev-parse HEAD", { encoding: "utf8" }).trim();
        const currentProvenance = JSON.parse(fs.readFileSync(".agent/provenance.json", "utf8"));
        currentProvenance.commit = commitHash;
        currentProvenance.hash = require("crypto").createHash("sha256").update(JSON.stringify(currentProvenance, Object.keys(currentProvenance).sort())).digest("hex");
