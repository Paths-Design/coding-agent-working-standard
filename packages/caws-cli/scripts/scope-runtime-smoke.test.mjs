...
test('installed scope guard evaluates root and nested targets in their bound lane', () => {
  ...
  const run = (command, args, cwd = repo, extra = {}) => {
    const result = observedSpawn(command, args, { cwd, env, encoding: 'utf8', timeout: 60000, ...extra });
  ...
  try {
    ...
    const spec = (id, scope) => cli(['specs', 'create', id, '--title', id,
      '--mode', 'fix', '--risk-tier', '3', '--activate', '--module', 'fixture',
      '--invariant', 'Only the bound spec decides scope', '--acceptance', 'Exercise scope decisions',
    ...
    for (const [target, admitted] of [['package.json', true], ['src/owned/ok.ts', true],
      ...
      assert.notEqual(canonical.decision, bound.decision, 'fixture must distinguish canonical and lane authority');
      const result = observedSpawn('bash', [path.join(repo, '.caws/hooks/scope-guard.sh')], {
        cwd: repo, env: { ...env, CAWS_PROJECT_DIR: repo, CAWS_AGENT_SURFACE: 'claude-code', HOOK_CWD: repo },
