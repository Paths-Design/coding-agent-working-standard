...
| `caws agents register / heartbeat / stop / list / show / work-state / prune` | Agent-liveness substrate in `.caws/leases/`; operational cache only. `work-state` sets a visibility-only annotation (`working / blocked_awaiting_human / review_ready / done`) on this session's lease — surfaced in `agents list`, the status Agents panel, and message sender-context; never authority. |
| `caws message send / reply / poll / inbox / history / status / prune` | Directed inter-agent messages over `.caws/messages.jsonl`; not authority. `--to` accepts `wt:<worktree>` / `spec:<spec-id>` aliases; a send to an idle (stopped-lease, fresh-heartbeat) recipient succeeds and surfaces at its next tool call; refusals print a not-sent verdict to stdout and are ledgered as `refusal` records; `reply <message_id>` answers on the same channel and writes `reply_to` linkage (`send --reply-to` validates the same); delivery records carry a receipt mode (`auto` for the heartbeat hook, `poll` for explicit polls); `inbox --all` lists every undelivered message repo-wide; `prune --apply` archives instead of deleting; `status <message_id>` observes queued-vs-delivered. |
