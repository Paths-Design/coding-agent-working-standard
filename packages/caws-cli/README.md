...
| `caws worktree create / list / ensure / bind / destroy / merge / migrate-registry / repair-sparse / repair` | Worktree lifecycle on the vNext substrate. Canonical path for parallel agent work. `ensure <name> --spec <id>` is the idempotent create-or-admit form (WORKTREE-ENSURE-AFFORDANCE-001): absent lanes are created via the full create path; existing same-spec untouched lanes admit with no new events; foreign-owned, different-spec, closed-spec, and moved-branch states refuse with their handoffs. |
