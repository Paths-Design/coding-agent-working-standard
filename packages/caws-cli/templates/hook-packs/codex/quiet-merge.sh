...
# Resolve repo root (may differ from CODEX_PROJECT_DIR in worktrees).
...
if echo "$COMMAND" | grep -qE 'caws\s+worktree\s+(merge|destroy)\b' && ! echo "$COMMAND" | grep -qE '[|>]'; then
  ...
  if command -v emit_updated_input >/dev/null 2>&1; then
    emit_updated_input "$QUIET_CMD"
  else
    printf '{"hookSpecificOutput":{"hookEventName":"PreToolUse","permissionDecision":"allow","updatedInput":{"command":%s}}}' "$(printf '%s' "$QUIET_CMD" | jq -Rs .)"
  fi
  exit 0
fi
