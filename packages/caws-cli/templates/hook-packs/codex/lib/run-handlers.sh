...
# Shared handler-dispatch loop for Codex hook dispatchers.
#
...
#   CODEX_HOOK_DRY_RUN   — if non-empty and non-zero, still invoke every
...
#   CODEX_HOOK_TIMING    — if non-empty and non-zero, emit
...
#               CODEX_HOOK_DRY_RUN is set the effective return is always 0.
#
...
run_handlers() {
  ...
  local dry_run=0
  _rh_is_truthy "${CODEX_HOOK_DRY_RUN:-}" && dry_run=1

  local timing=0
  _rh_is_truthy "${CODEX_HOOK_TIMING:-}" && timing=1
