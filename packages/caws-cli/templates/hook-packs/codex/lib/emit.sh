...
# Shared Codex hook-output envelope emitters.
#
# Codex reads a hook's stdout as JSON to decide block / deny /
...
# drift; if Codex's hook-output contract ever changes, it would be
...
#
#   2. ask              Codex PreToolUse does not support permissionDecision
#                       "ask" yet. Conservatively emit deny with an approval
#                       reason so a guard's ask-level escalation does not fail
#                       open.
#                       { "hookSpecificOutput": {
#                           "hookEventName": "PreToolUse",
#                           "permissionDecision": "deny",
#                           "permissionDecisionReason": <msg> } }
#                       PreToolUse conservative denial.
#
...
#       (Codex honors `decision: block` regardless, but the pack
