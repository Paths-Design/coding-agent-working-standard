...
# CAWS Command Safety Gate for Codex
...
danger_state_dir() {
  local project_dir="${CODEX_PROJECT_DIR:-.}"
  local state_dir="$project_dir/.codex/hooks/state"
  mkdir -p "$state_dir"
  printf '%s\n' "$state_dir"
}
...
SESSION_ID=$(printf '%s' "$INPUT" | jq -r '.session_id // env.CODEX_SESSION_ID // env.HOOK_SESSION_ID // "unknown"')
...
if [[ -f "$LATCH_FILE" ]]; then
  ...
  REASON="CAWS command-safety: a dangerous command was previously blocked or sent for approval in this Codex session. $TRIGGER_NOTE This is a human-review boundary, not a retryable syntax error. Do not rephrase, wrap, reorder, alias, or indirectly invoke the command. You, the agent, CANNOT clear this in-band: the reset is human-only by design AND a reset run from a latched session resolves no human shell session-id. Ask the USER to run, from their own shell (use --session with THIS session id, not --current): bash .codex/hooks/reset-danger-latch.sh --session $SESSION_ID --reason \"<why this is safe>\"  (or --all to clear every latch). Sentinel: $LATCH_FILE"
...
PROTECTED_HOOK_REL=".codex/hooks/worktree-write-guard.sh"
PROTECTED_HOOK_ABS="${CODEX_PROJECT_DIR:-.}/.codex/hooks/worktree-write-guard.sh"
if printf '%s' "$COMMAND" | grep -qF "$PROTECTED_HOOK_REL" || printf '%s' "$COMMAND" | grep -qF "$PROTECTED_HOOK_ABS"; then
  ...
  if printf '%s' "$COMMAND" | grep -qE '(^|[;&|[:space:]])git[[:space:]]+(add|commit|status|diff|log|show)\b'; then
    ...
    # NOTE: sed/perl are NOT here — they only mutate in place with -i (next arm).
    # HOOK-CAPABILITY-ENGINE-003: `sed -n 40,90p <guard>` is a READ (print mode).
    ...
    # sed/perl ONLY with an in-place flag write the file; without it they READ.
...
if [[ ! -f "$CLASSIFIER" ]] || ! command -v python3 >/dev/null 2>&1; then
  ...
  REASON="CAWS command-safety: command classifier unavailable; dangerous-command safety cannot verify Bash semantics. The session danger latch is NOW ARMED (fail-closed). $_LATCH_SCOPE_NOTE — you cannot reset it yourself. Ask the USER to run: bash .codex/hooks/reset-danger-latch.sh --session $SESSION_ID --reason \"<why this is safe>\". Command was: $COMMAND"
  emit_ask_json "$REASON"
  exit 0
fi
...
case "$DECISION" in
  ...
  ask)
    ...
    if [[ "$ENFORCEMENT" == "confirm" ]]; then
      ...
      # marker; the SECOND distinct capability ask LATCHES. A single capability
      # ask is routine and was the dominant false-positive freeze; a second in
      # the same session is the thrash signature the latch exists to stop.
      ...
      if [[ "$SOURCE" == "capability" ]]; then
        ...
        FULL_REASON="CAWS command-safety: $REASON. This is the SECOND capability-risk command this session — the first was a non-blocking warning; this one ARMS the session danger latch. $_LATCH_SCOPE_NOTE — you CANNOT reset it yourself. Do not rephrase, wrap, reorder, alias, or indirectly invoke the command to evade this. Ask the USER to confirm and run: bash .codex/hooks/reset-danger-latch.sh --session $SESSION_ID --reason \"<why this is safe / approved>\", then proceed. Command was: $COMMAND"
        emit_block_json "$FULL_REASON"
        exit 0
      fi
      record_danger_latch "$LATCH_FILE" "ask" "$REASON" "$COMMAND"
      FULL_REASON="CAWS command-safety: $REASON. This requires USER CONFIRMATION before it runs and the session danger latch is NOW ARMED (fail-closed: the classifier could not verify this command). $_LATCH_SCOPE_NOTE — you CANNOT reset it yourself. Do not rephrase, wrap, reorder, alias, or indirectly invoke the command to evade this. Ask the USER to confirm and run: bash .codex/hooks/reset-danger-latch.sh --session $SESSION_ID --reason \"<why this is safe / approved>\", then proceed. Command was: $COMMAND"
      emit_block_json "$FULL_REASON"
      exit 0
    fi
  ...
  *)
    ...
    FULL_REASON="CAWS command-safety: command classifier returned an unrecognized decision '$DECISION'. The session danger latch is NOW ARMED (fail-closed). $_LATCH_SCOPE_NOTE — you cannot reset it yourself. Ask the USER to run: bash .codex/hooks/reset-danger-latch.sh --session $SESSION_ID --reason \"<why this is safe>\". Command was: $COMMAND"
