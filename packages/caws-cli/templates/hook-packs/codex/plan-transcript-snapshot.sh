...
#   - $HOME/.codex/.pending-plan-snapshots — newline-separated list of
...
# Codex versions. Match: "file_path":"/.../.codex/plans/<name>.md"
#
# `|| true` swallows grep's exit-1 on no-match so set -e doesn't abort
# the script before our bail-out check runs.
PLAN_FILE=$(grep -oE '"file_path":"[^"]*/\.codex/plans/[^"]*\.md"' "$TRANSCRIPT_PATH" 2>/dev/null \
    | tail -1 \
    | sed -E 's/^"file_path":"//; s/"$//' \
    || true)
...
PENDING="$HOME/.codex/.pending-plan-snapshots"
