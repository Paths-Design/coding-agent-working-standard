...
#
# CAWS Scope Guard Hook for Codex (v11-shape).
...
emit_scope_progression() {
  ...
  local hard_block_guidance="If prior strikes from earlier edits are cornering this session and the scope is now correct, ask the user to run: bash .codex/hooks/reset-strikes.sh --current (or --session <uuid>) to clear stale strike state. Verify the worktree binding: the spec must declare 'worktree: <name>' and .caws/worktrees.json must map that same worktree name to the correct 'specId' (v10) or 'spec_id' (v11). On CAWS v11.0 the worktree lifecycle CLI is not yet restored; on v11.1+ use 'caws worktree bind'. Do not edit .codex/hooks/, .codex/logs/guard-strikes-*.json, or other guard state to bypass this check."
...
POLICY_FILE="${CODEX_PROJECT_DIR:-.}/.caws/policy.yaml"
...
WORK_DIR="${HOOK_CWD:-${CODEX_PROJECT_DIR:-.}}"
...
# always-allowed ABSOLUTE prefix (e.g. `$HOME/.codex/` — the harness's own
...
# writing its own `~/.codex/.../memory/*.md`. Honor absolute allow-prefixes
