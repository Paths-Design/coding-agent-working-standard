...
#
# CAWS Naming Convention Check Hook for Codex
#
...
# the hook enforces remains: AGENTS.md key rule "No shadow files —
...
for modifier in "${BANNED_MODIFIERS[@]}"; do
  ...
  if [[ "$FILENAME_LOWER" =~ (^|[-_.])"$modifier"([-_.]|$) ]]; then
    ...
    emit_additional_context "Warning: The filename '$FILENAME' contains the modifier '$modifier' which may indicate temporary or non-canonical naming. Per the CAWS doctrine (AGENTS.md key rule \"No shadow files\"), edit existing files in place rather than creating *-enhanced.*, *-new.*, *-v2.* copies. Consider using a more descriptive, permanent name." "PostToolUse"
    exit 0
  fi
done
