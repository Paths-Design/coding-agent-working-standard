...
# CAWS Codex Hook Pack — Inventory

Human-facing inventory of the v11 Codex hook pack. For the **agent
doctrine** (managed-file rules, spec-authoring traps, v10/v11 state model,
`.codex/hooks.json` wiring) see [`AGENTS.md`](./AGENTS.md) in this directory.
...
`caws init --agent-surface codex`. Hand-editing a managed file turns it
into an unmanaged snowflake the installer will then refuse to touch.

## How the pack runs

Codex reads `.codex/hooks.json` from trusted project config layers and invokes
five dispatchers under `caws_dispatch/`. Each reads stdin once via
`lib/parse-input.sh`, then fans out to a registered handler list via
`lib/run-handlers.sh`.

Project-local Codex hooks do not run until the project `.codex/` layer is
trusted. New or changed non-managed hook definitions must also be reviewed and
trusted with `/hooks`.

| Dispatcher | Codex event | Matcher | Timeout |
|---|---|---|---|
| `caws_dispatch/session_start.sh` | `SessionStart` | startup, resume, clear, compact | 30 s |
| `caws_dispatch/pre_tool_use.sh` | `PreToolUse` | Bash, apply_patch, Edit, Write | 45 s |
| `caws_dispatch/post_tool_use.sh` | `PostToolUse` | Bash, apply_patch, Edit, Write | 60 s |
| `caws_dispatch/pre_compact.sh` | `PreCompact` | manual, auto | 30 s |
...
| `protected-paths.sh` | Write, Edit | Blocks hook **scripts** under `.codex/hooks/` (`*.sh`/`*.py`/`*.cjs`, exit 1) and strike-state `.codex/logs/guard-strikes-*.json` (exit 2). Documentation (`*.md`) under `.codex/hooks/` is admitted; every other extension stays blocked (fail-closed). |
...
Codex reports file edits through `apply_patch`; `lib/parse-input.sh` maps those
payloads to CAWS-style `Write`/`Edit` variables for the guard scripts. Codex
does not currently support PreToolUse `ask`; ask-level CAWS escalations emit a
conservative `permissionDecision: "deny"` instead of failing open.
...
| `lib/run-handlers.sh` | five dispatchers | the handler fan-out loop + exit-code aggregation |
| `lib/caws-state.sh` | state-reading hooks | v10/v11 dual-shape registry + canonical-root resolution |
| `lib/emit.sh` | hooks that emit envelopes | the three Codex hook-output envelope shapes |
...
4. If strikes are stale after a legitimate scope fix, ask the user to run
   `bash .codex/hooks/reset-strikes.sh --current`.
5. See [`AGENTS.md`](./AGENTS.md) for the full doctrine.
