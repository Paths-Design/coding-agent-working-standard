...
"""
Command safety classifier for Codex PreToolUse hooks.
...
def _trusted_git_init_token_path(repo_root: Path) -> Path | None:
    ...
    if not re.match(r"^[A-Za-z0-9._-]{8,128}$", nonce):
        return None
    return repo_root / ".codex" / "hooks" / "state" / f"allow-git-init-{nonce}"
...
def main() -> None:
    ...
    parser.add_argument("--repo-root", default=os.environ.get("CODEX_PROJECT_DIR", "."))
