...
#
# CAWS Worktree Safety Guard for Codex (v11-shape).
...
PROJECT_DIR="$(resolve_canonical_dir "${CODEX_PROJECT_DIR:-.}")"
...
if [[ -n "$BASE_BRANCH" ]] && [[ "$CURRENT_BRANCH" == "$BASE_BRANCH" ]]; then
  ...
  if echo "$COMMAND" | grep -qE 'git\s+commit\b' && ! echo "$COMMAND" | grep -qE '--amend'; then
    emit_additional_context "NOTE: committing to the base branch ($BASE_BRANCH) while worktrees are active. Worktrees are preferred for isolated feature work, but logical checkpoint commits from the current checkout are allowed by Codex hooks. Avoid --amend and force-push while worktrees are active."
    exit 0
  fi
fi
