...
# Shared progressive strike handling for Codex pre-write guard hooks.
#
# STRIKE-STATE KEYING (matches AGENTS.md "Scope-guard strike state"):
# The counter is keyed by (session_id, checkout-location), NOT session-global.
#   - canonical checkout  -> $CODEX_PROJECT_DIR/.codex/logs/guard-strikes-<session>.json
...
# canonical .codex/logs file. This per-checkout isolation is intentional —
# cross-worktree strike bleed is a high-severity control-plane collapse risk.
# (If a block says "strike 3" but the canonical .codex/logs
...
# reason is logged to .codex/logs/strike-resets.log.
...
guard_worktree_state_dir() {
  ...
  # worktree). Signal the caller to fall back to the canonical .codex/logs
