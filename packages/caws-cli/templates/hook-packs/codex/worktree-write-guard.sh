...
#
# CAWS Worktree Write Guard for Codex.
#
...
#      and asserts the always-allowed allowlist so .caws/, .codex/,
...
# predicate.
WORKTREE_ROOT="${CODEX_PROJECT_DIR:-.}"
...
#
# WORKTREE_ROOT is the cwd-as-resolved-via-CODEX_PROJECT_DIR. NOT
...
# User-global Codex state lives outside the repo; .caws/, .codex/, docs/,
...
if [[ -n "$FILE_PATH" ]]; then
  ...
  case "$FILE_PATH_FOR_ALLOWLIST" in
    ...
    "$PROJECT_DIR"/.codex/*|.codex/*) exit 0 ;;
    # Root AGENTS.md is the project-level agent-instruction surface; it lives
    # at the repo root (not under .codex/) so it needs its own arm.
    "$PROJECT_DIR"/AGENTS.md|AGENTS.md) exit 0 ;;
...
# CODEX_PROJECT_DIR (which always points to the main repo root, even when the
# agent has cd'd into a worktree at .caws/worktrees/<name>/).
AGENT_DIR="${HOOK_CWD:-${CODEX_PROJECT_DIR:-.}}"
  ...
  SPEC_CONTENTION_CHECK=$(PROJECT_DIR="$PROJECT_DIR" CURRENT_BRANCH="$CURRENT_BRANCH" REL_PATH="$REL_PATH" node -e "
    ...
    try {
      ...
      for (var wi = 0; wi < worktrees.length; wi++) {
        ...
        // documentation, not a hostility surface (AGENTS.md trap #4). This
...
_guard_risk_reason() {
  ...
  if [[ "$WT_COUNT" -eq 0 ]] 2>/dev/null; then
    body="No active worktree is present for branch '$CURRENT_BRANCH'. Worktrees are preferred for isolated feature work; repo-coordination paths (.caws/, .codex/, docs/) are already allowlisted above."
  else
...
if _guard_no_ask; then
  ...
  echo "Do NOT edit .codex/hooks/ or guard state to bypass this. Ask the user if a base-branch edit is genuinely needed." >&2
  exit 2
fi
