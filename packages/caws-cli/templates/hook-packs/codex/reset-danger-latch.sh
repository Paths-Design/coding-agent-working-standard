...
# .codex/hooks/state/ and records each reset (with a mandatory reason)
# to .codex/logs/danger-latch-resets.log.
...
LOG_FILE="$PROJECT_DIR/.codex/logs/danger-latch-resets.log"
...
Modes (exactly one required):
  --current            Clear the latch for the current Codex session
                       (resolved from CODEX_SESSION_ID / HOOK_SESSION_ID).
...
case "$MODE" in
  current)
    SESSION_ID="${CODEX_SESSION_ID:-${HOOK_SESSION_ID:-unknown}}"
    ...
    if [[ -f "$CANDIDATE" ]]; then
      ...
      # that has no Codex session id in its env, so SESSION_ID resolves to
