...
if (require.main === module) {
  ...
  const allowlistArg = process.argv.find((arg) => arg.startsWith('--allowlist='))?.split('=')[1];
  const allowlistPath = allowlistArg || '.agent/tools-allow.json';

  if (promptFiles.length === 0) {
    console.log('CAWS Prompt Linter');
    console.log('Usage: node prompt-lint.js <prompt-file1> [prompt-file2] ... [options]');
    console.log('Options:');
    console.log(
  ...
  if (results.secrets.length > 0) {
    ...
    results.secrets.forEach((secret, index) => {
      console.log(
        `  ${index + 1}. ${secret.file}:${secret.line} - ${secret.match.substring(0, 50)}...`
      );
    });
  }
