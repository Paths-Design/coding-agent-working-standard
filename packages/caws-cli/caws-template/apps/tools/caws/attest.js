...
function generateSBOM(projectPath = '.') {
  ...
  try {
    ...
    const sbom = {
      ...
      dataLicense: 'CC0-1.0',
      SPDXID: 'SPDXRef-DOCUMENT',
      documentNamespace: `https://caws.dev/sbom/${packageJson.name || 'unknown'}-${Date.now()}`,
      packages: [
        {
    ...
    if (packageJson.dependencies) {
      Object.entries(packageJson.dependencies).forEach(([name, version]) => {
        const packageId = `SPDXRef-Package-${name.replace(/[^a-zA-Z0-9]/g, '-')}`;
    ...
    if (packageJson.devDependencies) {
      Object.entries(packageJson.devDependencies).forEach(([name, version]) => {
        const packageId = `SPDXRef-Package-Dev-${name.replace(/[^a-zA-Z0-9]/g, '-')}`;
    ...
    byproducts: [
      {
        name: 'sbom.json',
        digest: {
          sha256: crypto.createHash('sha256').update(JSON.stringify(sbom)).digest('hex'),
        },
      },
    ],
  });
