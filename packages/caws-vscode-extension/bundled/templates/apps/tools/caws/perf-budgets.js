...
exports.PerformanceBudgetValidator = void 0;
...
class PerformanceBudgetValidator {
    ...
    async validateBudgets(useRealData = false) {
        const results = [];
        ...
        if (!overall_passed) {
            ...
            results
                .filter((r) => !r.passed)
                .forEach((r) => {
                summary += `\n  • ${r.endpoint}: ${r.p95_ms}ms > ${r.budget_ms}ms budget (${r.deviation_percent.toFixed(1)}% over)`;
            });
        }
        else {
    ...
    getRealPerformanceMeasurements() {
        try {
...
async function main() {
    ...
    try {
        ...
        console.log('=====================================');
        console.log();
        console.log(`📊 Data Source: ${useRealData ? 'Real Performance Data' : 'Mock Data (CI/Development)'}`);
        console.log();
        ...
        if (validator['budgets'].speech_processing_per_second) {
            console.log(`  • Speech processing: ${validator['budgets'].speech_processing_per_second} sec/sec`);
        }
        console.log();
        console.log('📈 Validation Results:');
        validation.results.forEach((result) => {
            const status = result.passed ? '✅' : '❌';
            const deviation = result.deviation_percent > 0 ? `(+${result.deviation_percent.toFixed(1)}%)` : '';
...
//# sourceMappingURL=perf-budgets.js.map