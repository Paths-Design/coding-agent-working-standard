...
exports.GatesCLI = void 0;
const gate_checker_js_1 = require("./shared/gate-checker.js");
...
class GatesCLI {
    constructor() {
        this.gateChecker = new gate_checker_js_1.CawsGateChecker();
        this.configManager = new config_manager_js_1.CawsConfigManager();
    }
    /**
     * Show tier policy
     */
    showTierPolicy(tier = 1) {
     ...
     */
    async enforceCoverageGate(tier = 2) {
        try {
            const policy = TIER_POLICIES[tier];
            ...
            if (result.passed) {
                console.log(`✅ Coverage gate passed: ${(result.score * 100).toFixed(1)}% ≥ ${policy.branch_coverage * 100}%`);
                return true;
            }
            else {
                console.log(`❌ Coverage gate failed: ${(result.score * 100).toFixed(1)}% < ${policy.branch_coverage * 100}%`);
     ...
     */
    async enforceMutationGate(tier = 2) {
        try {
            const policy = TIER_POLICIES[tier];
            ...
            if (result.passed) {
                console.log(`✅ Mutation gate passed: ${(result.score * 100).toFixed(1)}% ≥ ${policy.mutation_score * 100}%`);
                return true;
            }
            else {
                console.log(`❌ Mutation gate failed: ${(result.score * 100).toFixed(1)}% < ${policy.mutation_score * 100}%`);
                if (result.errors && result.errors.length > 0) {
                    result.errors.forEach((error) => console.error(`  - ${error}`));
                }
                return false;
     ...
     */
    async enforceContractGate(tier = 2) {
        try {
