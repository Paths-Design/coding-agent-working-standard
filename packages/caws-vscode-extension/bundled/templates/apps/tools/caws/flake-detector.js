...
exports.FlakeDetectionService = void 0;
...
class FlakeDetectionService {
    constructor() {
        this.HISTORY_FILE = '.caws/flake-history.json';
        this.QUARANTINE_FILE = '.caws/quarantined-tests.json';
        this.VARIANCE_THRESHOLD = 0.05; // 5% variance threshold
        this.MIN_RUNS_FOR_ANALYSIS = 3;
        this.QUARANTINE_THRESHOLD = 0.15; // 15% flake rate triggers quarantine
    }
    /**
     * Analyze test variance and detect flaky tests
     */
    async detectFlakes(currentResults) {
    ...
    quarantineTests(testNames) {
        ...
        (0, fs_1.writeFileSync)(this.QUARANTINE_FILE, JSON.stringify(quarantinedData, null, 2));
     ...
     */
    releaseFromQuarantine(testNames) {
        const history = this.loadHistory();
    ...
    loadHistory() {
        if (!(0, fs_1.existsSync)(this.HISTORY_FILE)) {
            return {
                runs: [],
                quarantined: new Set(),
        ...
        try {
            const data = JSON.parse((0, fs_1.readFileSync)(this.HISTORY_FILE, 'utf-8'));
            return {
                runs: data.runs || [],
    ...
    saveHistory(history) {
        ...
        (0, fs_1.writeFileSync)(this.HISTORY_FILE, JSON.stringify(data, null, 2));
    }
    createCurrentRun(results) {
    ...
    calculateFlakeRate(testName, runs) {
    ...
    calculateVarianceScore(history) {
    ...
    calculateRunVariance(testMap, _suites) {
    ...
    generateRecommendations(flakyTests, varianceScore) {
...
exports.FlakeDetectionService = FlakeDetectionService;
/**
 * CLI Interface
 */
async function main() {
...
//# sourceMappingURL=flake-detector.js.map