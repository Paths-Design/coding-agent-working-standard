...
Currently absent: `caws claim --spec <id>` (bridge claims for non-worktree contexts) and `caws worktree prune/reconcile` (the broad reconcile over ambiguous classes — the *unambiguous* half-state subset shipped as `caws worktree repair`). `caws agents list/show` ships in v11.1 for read-only agent lease inspection.
...
- v11.1 **does** ship the worktree lifecycle commands: `caws worktree create | list | bind | destroy | merge | migrate-registry | repair-sparse | repair`. Use them — do not fall back to raw `git worktree` / `git merge` by default. (`caws worktree repair` is the governed half-state executor — it prunes ghost registry entries and clears dead spec→worktree bindings for the unambiguous classes the doctor surfaces; distinct from `repair-sparse`, which restores the `.caws/specs` sparse-checkout invariant.) In particular, **`caws worktree merge <name>` is the governed merge path**: it runs `git checkout <base>` + `git merge --no-ff` + auto-closes the bound spec (`spec_closed`) + appends `worktree_merged`, all as one transaction over the v11 flat-map `worktrees.json`. Prefer it over a manual `git checkout main && git merge --no-ff` — the manual base checkout is a bare `git checkout <existing-branch>`, which the danger-latch classifier flags as potentially discarding work (it auto-admits only `checkout -b`), so doing the merge by hand can trip the latch and require a human reset. The governed command does the base checkout *inside* the transaction and avoids that trap. (The old `WORKTREE-MERGE-V11-SHAPE-001` registry-shape crash that once forced the raw-git fallback is fixed — `caws worktree merge` reads the flat-map registry natively.)
