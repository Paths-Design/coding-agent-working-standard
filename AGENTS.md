...
**The v11 line ships fourteen top-level commands/groups** (plus the auto-generated `help`): the governed core plus `specs`, `worktree`, `events`, `agents`, `message`, and `reprieve`. Run `caws --help` for the authoritative list. Commands removed and not planned to return (`validate`, `verify-acs`, `evaluate`, `iterate`, `diagnose`, `burnup`, `provenance`, `hooks`, `scaffold`, `mode`, `tutorial`, `plan`, `workflow`, `quality-monitor`, `tool`, `test-analysis`, `templates`, `sidecar` at the v11.0 cutover; `prepush` after it) will fail if invoked. `caws agents list/show` **ship in v11.1** (liveness substrate). `caws message` is a directed message channel, not authority. Provenance teeth live at the merge boundary: `caws worktree merge` refuses a lane whose commits touch paths outside the bound spec's scope, and records the landing in a `worktree_merged` event. Only `caws session` and `caws parallel` are deferred (v11.3+); bridge claims and lease-backed *authority* are the v11.2 plan.
