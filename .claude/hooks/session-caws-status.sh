...
if [ -f "$CAWS_ROOT/.caws/worktrees.json" ] && command -v node >/dev/null 2>&1; then
  ...
  if [ "$WT_COUNT" -gt 0 ] 2>/dev/null; then
    ...
    if [ -n "$BASE_BRANCH" ] && [ "$CURRENT_BRANCH" = "$BASE_BRANCH" ]; then
      ...
      echo ""
      echo "  Useful operations on $CURRENT_BRANCH include:"
      echo "    - git merge --no-ff <branch> (merge completed worktree work)"
      echo "    - Normal logical checkpoint commits for current work"
      echo "    - wip(checkpoint): <description> for prior-session dirty files"
      echo ""
      echo "  Writing or editing files on $CURRENT_BRANCH is no longer blocked"
      echo "  by the PreToolUse hook."
    else
      echo ""
