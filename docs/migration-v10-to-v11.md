...
| `caws verify-acs`               | `caws specs verify-acs <id>` (12.2+)                                                                             | Restored under `specs` (`CAWS-SPECS-VERIFY-ACS-REDERIVE-001`). Re-derives the spec's recorded `evidence[]` block — cited `commit_sha` (exists, reachable), `artifact_path` (present at that revision), `test_nodeid` (exists; with `--run`, executes) — with three verdicts that are never collapsed: `verified` / `refuted` / `not_rederived`. v10.2 reported a collected-but-unexecuted test as PASS; that is now `not_rederived`, and a criterion with only narrative evidence is counted as unverifiable, never as passing. A recorded `command` is never executed. `--strict` fails on any `not_rederived`; `--json` emits a `verify-acs.v1` envelope. |
