...
`prepush --ack` bundles, and `gates run <spec-id>`. Treat stale fixed
...
`gates run <spec-id>`) as closed and select the newest current leaf mismatch
for the next concrete fix, if any.
