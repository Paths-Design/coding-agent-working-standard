...
### Post-Handoff Authority Resample

After the active-spec handoff and JSON repair-field slices, I resampled the
same three transcript locations for residual `scope.no_authority` / `NO
AUTHORITY` evidence. The logs still contain historical failures, including a
recent Sterling transcript copied before this build was available, but direct
linked-dist reproduction from Sterling's canonical checkout now emits the
enriched authority model instead of the old generic `repair` field.

| Source | Files scanned | Files with scope/authority evidence | Authority markers | Newest relevant evidence |
|---|---:|---:|---:|---|
| CAWS `.caws/sessions` | 229 | 67 | 271 | scope command transcripts through 2026-06-26; no newer generic no-authority repair after the handoff slices |
| CAWS `tmp/` rendered sessions | 854 | 244 | 1,366 | old generic unbound repair text from 2026-05-30 |
| Sterling `.caws/sessions` | 4,728 | 364 | 649 | stale copied generic unbound repair text from 2026-07-04; current linked-dist reproduction now shows active-spec candidates and `authorityCandidates` JSON |

Current linked binary check: `command -v caws` resolves through
`packages/caws-cli/dist/index.js`, and from Sterling an unbound `caws scope
check <path>` now prints active spec candidates, read-only `caws scope show
<path> --spec <id>` checks, and worktree create/enter handoffs. JSON output
...
The follow-up slice fixed the governed scope-widening mismatch as well:
...
| `unknown_or_missing_option` | 91 | Further fixed for `specs close`, `scope`, `agents prune`, `specs create`, validation-era removed commands, specs status listing, worktree cleanup state filters, specs archive/prune-archive discoverability, governed scope amendment rationale, worktree destroy compatibility, init preview compatibility, prepush ack bundle compatibility, gates-run spec targeting, and group-level worktree prune handoff: `--closure-notes`, `--notes`, and `--note` are now supported aliases for close notes, `scope show/check/plan --spec <id>` supports read-only explicit spec-context evaluation, `agents prune --dead --json` is verified, `specs create --tier <n>` aliases `--risk-tier <n>`, `specs create --id <id>` aliases the positional create id, legacy validation diagnostics are pinned, `specs --status <state>` hands off to `specs list --status <state>`, `worktree prune/cleanup-plan --status <classes>` aliases `--state <classes>`, `worktree --prune ...` normalizes to `worktree prune ...`, `worktree destroy --force` aliases `--abandon-unmerged` only, `init --dry-run` aliases `init --plan`, shell-quoted `prepush "--ack <sha> --ack <sha>"` bundles normalize to repeated `--ack`, `gates run <spec-id>` aliases `gates run --spec <id>`, `specs prune-archive` hands off to archive/restore/recover workflows, and `specs amend-scope --reason <text>` records rationale on `spec_scope_amended` evidence. Remaining work should resample the newest option mismatches after these fixes before selecting another leaf. |
...
| Scope from canonical checkout | `NO AUTHORITY scope.no_authority.unbound ... No spec is bound to this worktree`. | Closed and resampled: historical transcripts still contain the old generic repair text, but current linked-dist output lists active spec authority candidates, includes read-only `caws scope show <path> --spec <id>` comparisons, and hands off to `worktree create`, `worktree bind`, or `cd .caws/worktrees/<name>` depending on whether the candidate spec is already bound. |
...
| Closure notes flag mismatch | `Unknown option: --closure-notes` / `unknown option '--notes'` / `unknown option '--note'` when an agent tried to close a spec using the YAML field name or natural closure-note shorthand. | Now closed for `specs close`: `--closure-notes <text>`, `--notes <text>`, and `--note <text>` alias `--reason <text>`, all write `closure_notes`, and supplying more than one fails before mutation. |
...
| Gates run positional spec mismatch | `caws gates run REALIZATION-FR-FUTURE-NEGATION-POLICY-RECON-01` returned `error: required option '--spec <id>' not specified` even though the spec id is the natural object of the run leaf. | Now closed for `gates run`: an optional positional `<spec>` aliases `--spec <id>`, both forms use the same governed gate execution path, and supplying both refuses before policy evaluation or event writes. |
...
| Worktree group prune mismatch | `caws worktree --prune` returned `unknown option '--prune'` while the adjacent cleanup model already existed as the `worktree prune` leaf. | Now closed by argv normalization: `caws worktree --prune ...` is rewritten to `caws worktree prune ...` before Commander parsing, preserving the existing prune leaf's dry-run/apply, selector, JSON, and ambiguity semantics. |
...
| Amend-scope reason mismatch | `unknown option '--reason'` when using the documented audit-style `caws specs amend-scope ... --reason "scope narrowed"` example. | Now closed for `specs amend-scope`: `--reason <text>` is accepted, surfaced in nested help and docs, and recorded on the appended `spec_scope_amended` event payload. |
...
| `UX-SPECS-CLOSE-NOTE-ALIAS-001` | Implemented in sixtieth repair slice | Specs close singular note alias | Adds `caws specs close <id> --note <text>` as a singular compatibility alias for closure notes alongside `--reason`, `--closure-notes`, and `--notes`. Supplying more than one note alias fails before mutation; spawned CLI coverage proves Commander no longer rejects `--note`. Covered by `packages/caws-cli/tests/shell/specs-close-closure-notes-alias.test.js`. |
...
| `UX-GATES-RUN-POSITIONAL-SPEC-001` | Implemented in sixty-second repair slice | Gates run positional spec id alias | Adds `caws gates run <spec-id>` as a compatibility alias for `caws gates run --spec <id>`, keeping the same gate execution path and refusing ambiguous invocations that supply both forms before mutation. Covered by `packages/caws-cli/tests/shell/gates-run-positional-spec.test.js`. |
| `UX-WORKTREE-GROUP-PRUNE-HANDOFF-001` | Implemented in sixty-third repair slice | Worktree group-level prune handoff | Normalizes `caws worktree --prune ...` to `caws worktree prune ...` before Commander parsing, so git-style prune muscle memory reaches the existing dry-run/apply cleanup leaf without parent-option capture. Covered by `packages/caws-cli/tests/shell/worktree-group-prune-handoff.test.js`. |
...
`prepush --ack` bundles, `gates run <spec-id>`, and `worktree --prune`. Treat stale fixed
examples (`--tier`, `--status`, `--closure-notes`, `--notes`, `--note`,
...
`init --dry-run`, shell-quoted `prepush --ack` bundles, and positional
`gates run <spec-id>`, plus `worktree --prune`) as closed and select the newest
...
24. **Evidence JSON parse failures now name the repair path.**
   `evidence record --data` still refuses malformed JSON, but the refusal now
...
26. **Malformed hook input is fail-open but no longer invisible.**
   The shared hook parser still converts unreadable hook stdin into an empty
   payload so dispatchers do not block a legitimate tool call because the
   harness payload was malformed. It now emits a controlled CAWS diagnostic to
   stderr, avoids Python tracebacks, and never echoes the raw malformed input.

27. **Specs migration mapping parse failures now show the required file shape.**
   `specs migrate --lifecycle-mapping` still refuses malformed or wrongly shaped
...
30. **Residual no-authority transcripts are stale against current linked dist.**
   A post-handoff resample still finds old `scope.no_authority` snippets in
   CAWS `tmp/` and Sterling session logs, but direct reproduction from Sterling
   with the linked `packages/caws-cli/dist/index.js` binary now shows the
...
31a. **Specs close now accepts the singular `--note` shorthand too.**
   The residual resample found older generated commands using `--note` instead
   of `--notes`. `--note` now aliases the same `closure_notes` storage path and
   participates in the same "pass only one note alias" refusal.

32. **Specs amend-scope now records an audit reason.**
   The CLI docs already showed `caws specs amend-scope ... --reason "scope
...
37. **Gates run accepts the spec id as the leaf object.**
   Resampling showed remaining current retries where agents used
   `caws gates run <spec-id>` after reading validation-era guidance. The leaf
   now accepts optional positional `<spec>` as an alias for `--spec <id>`, while
   supplying both forms refuses before policy evaluation or gate evidence writes.

38. **Worktree group prune reaches the cleanup leaf.**
   Resampling showed a group-level `caws worktree --prune` retry. The entrypoint
   now normalizes that shape to `caws worktree prune` before Commander parsing,
   so selectors, JSON, dry-run/apply behavior, and state/status ambiguity are
   all owned by the existing prune leaf.
