...
**Canonical-drift guards (CANONICAL-DRIFT-GUARDS-001, failure-lineage Entry 37).** The lifecycle commands that auto-commit (`specs create/activate/amend-scope/close`) check the commit target BEFORE writing: when the canonical HEAD is parked on a non-base branch while CAWS worktrees are active, they refuse pre-write (nothing lands) unless `--allow-foreign-branch` names the deliberate choice; the healthy on-base state is byte-inert. `caws doctor` surfaces the condition as a WARN `doctor.canonical.mis_parked_head` finding (pure kernel, store-observed branch pair, suppressed on zero worktrees or observation failure). Recovery is `caws specs relocate <id> [--apply]`: object-db plumbing (read the parked copy, graft onto base via a private temp index, `commit-tree`, CAS the base ref with bounded retry) that never touches any working tree; the durable audit is the relocation commit on base itself.
...
| `caws specs amend <id>` | Governed body amendments: `blast_radius.modules`, `invariants`, and acceptance criteria (`--set-ac`/`--add-ac`/`--remove-ac` with `--given`/`--when`/`--then`; one acceptance op per invocation, ids match `A<digits>`). Rewriting a criterion's text resets that criterion's evidence entry to `unchecked` in the same transaction (`reset_evidence` carries the previous status) — recorded proof never outlives the text it proved; removing a criterion deletes its evidence entry. Closed specs: blank-filling only (the scaffold discharge refuses over recorded evidence); a claim rewrite is refused with a `reopen` pointer. CAWS-SPEC-AMEND-ACCEPTANCE-001. |
