...
> **v11 surface.** The current v11 line ships full worktree lifecycle: `caws worktree create | list | bind | destroy | untrack | merge | migrate-registry | repair-sparse | repair | prune | cleanup-plan`. Use these commands — they write bidirectional bindings, emit audit events, and enforce ownership. (`caws worktree repair` is the governed half-state executor for ghost registry entries and dead spec→worktree bindings; `repair-sparse` restores the `.caws/specs` sparse-checkout invariant — the two are unrelated. `untrack` releases the registry binding while keeping the directory; `prune`/`cleanup-plan` are dry-run-by-default cleanup planners, registry and physical respectively.) `caws parallel setup` does not exist and is not planned; loop `caws worktree create` per spec instead.
